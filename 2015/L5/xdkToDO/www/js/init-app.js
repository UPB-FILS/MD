...
// Singleton instance - use the same database each time you re-start the application

app.initCBLite = function(){
    var fName = "initCBLite";
    if (window.cblite) {
            window.cblite.getURL(function(err, url) {
                if (err) {
                    app.consoleLog(fName,"error launching Couchbase Lite: " + err);
                } else {
                    app.consoleLog(fName,"Couchbase Lite running at " + url);
                }
            });
    } else {
        app.consoleLog(fName,"error, Couchbase Lite plugin not found.");
    }
};